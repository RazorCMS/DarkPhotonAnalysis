...
//    system("mkdir -p SplusB_fits");   
    ...
    for (int i = 0; i < 5; i++) {
         ...
         * Define signal shape
         */ 
        TString bwfit = MakeBreitWigner(Form("BW_sig%d", i), mlist[i], 0.001, mzd, *w);
        TString cbfit = MakeCB(Form("CB_sig%d", i), mlist[i], mzd, *w);
        TString dcbfit = MakeDoubleCB(Form("DCB_sig%d", i), mlist[i], mzd, *w);
        TString dcbfit_NE = MakeDoubleCB_NE(Form("DCB_NE_sig%d", i), mlist[i], mzd, *w);

        /*
         * Define background shape
         */ 
        TString dexpofit = MakeDoubleExpo(Form("DExpo_bkg%d", i), mzd, *w);
        TString dexpofit_NE = MakeDoubleExpo_NE(Form("DExpo_NE_bkg%d", i), mzd, *w);
        TString expofit = MakeExpo(Form("Expo_bkg%d", i), mzd, *w);
        TString bernpoly2fit = MakeBernPoly2(Form("BernPoly2_bkg%d", i), mzd, *w);
        TString bernpoly2fit_NE = MakeBernPoly2_NE(Form("BernPoly2_NE_bkg%d", i), mzd, *w);
        TString bernpoly3fit = MakeBernPoly3(Form("BernPoly3_bkg%d", i), mzd, *w);
        TString bernpoly3fit_NE = MakeBernPoly3_NE(Form("BernPoly3_NE_bkg%d", i), mzd, *w);
...
int SplusB_fit_test(TTree* tree) {
    ...
    TFile *file = new TFile("LowMassFits.root", "recreate");
    RooWorkspace *w = new RooWorkspace("LowMassFits", "");
     ...
     */ 
/*
    TString bwfit = MakeBreitWigner(Form("BW_sig%d", i), mass4, 0.001, mzd, *w);
    TString cbfit = MakeCB(Form("CB_sig%d", i), mass4, mzd, *w);
*/
    TString dcbfit = MakeDoubleCB(Form("DCB_sig%d", i), mass4, mzd, *w);

    TString dcbfit_NE = MakeDoubleCB_NE(Form("DCB_NE_sig%d", i), mass4, mzd, *w);
...
/*
    ...
    TString bernpoly3fit = MakeBernPoly3(Form("BernPoly3_bkg%d", i), mzd, *w);
    TString bernpoly3fit_NE = MakeBernPoly3_NE(Form("BernPoly3_NE_bkg%d", i), mzd, *w);
    TString bernpoly4fit_NE = MakeBernPoly4_NE(Form("BernPoly4_bkg%d", i), mzd, *w);
    TString bernpoly5fit_NE = MakeBernPoly5_NE(Form("BernPoly5_bkg%d", i), mzd, *w);
*/
...
//        RooFitResult *dcbfitres = w->pdf(dcbfit_NE)->fitTo(*massCut4, RooFit::Save(kTRUE), RooFit::Range("sig"), RooFit::Timer(1), RooFit::Strategy(2));
...
//        RooFitResult *dexpofitres_NE = w->pdf(dexpofit_NE)->fitTo(*massCut4, RooFit::Save(kTRUE), RooFit::Range("low,high"));
...
/*
    if (fit_file.is_open()) {
        ...
        fit_file << dcbfitres << std::endl;
        dcbfitres->printValue(fit_file);
        dcbfitres->printArgs(fit_file);
...
*/
    ...
    w->pdf(dcbfit_NE_old)->plotOn(frameS, RooFit::Name("dcbfit_NE_old"), RooFit::LineColor(kBlue));
    w->pdf(dcbfit_NE)->plotOn(frameS, RooFit::Name("dcbfit_NE"), RooFit::LineColor(kGreen));
    frameS->Draw();
    frameS->SetTitle("");
